...
CREATE TABLE frames (
    ...
    elevation REAL,           -- Elevation angle in degrees (-1.0 for volumetric products)
    metadata TEXT,            -- JSON string containing additional frame metadata
...
```sql
SELECT station, product_name, timestamp 
FROM frames 
